...
def define_customer_schema() -> list:
    """Defines and returns the schema for the customer BigQuery table."""
    return [
...
def transform_data(csv_data: list, json_data: list, xml_data: list) -> list:
...
def main(file_paths: list, project_id: str, dataset_id: str):
    """Main function to run the ETL process sequentially."""

    # --- Extract ---
    print("--- Starting Extraction Step ---")
    csv_data, json_data, xml_data = None, None, None
    for path in file_paths:
        file_format = path.split(".")[-1].lower()
        print(f"Extracting data from: {path}")
        if file_format == "csv":
            csv_data = extracts(path)
        elif file_format == "json":
            json_data = extracts(path)
        elif file_format == "xml":
            xml_data = extracts(path)
    print("--- Extraction Complete ---")

    # --- Transform ---
    print("\n--- Starting Transformation Step ---")
    transformed_data = transform_data(csv_data, json_data, xml_data)
    print("--- Transformation Complete ---")
...
file_paths = [
    "/Users/user/Code/ETL_Project/data/customers_billing.xml",
    "/Users/user/Code/ETL_Project/data/customers.csv",
    "/Users/user/Code/ETL_Project/data/customers_subscriptions.json",
]
...
if __name__ == "__main__":
    ...
    main(file_paths, PROJECT_ID, DATASET_ID)
